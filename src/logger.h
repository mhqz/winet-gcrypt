...
#define LOG_ERROR(...) do { if (logger.get_threshold() <= ERROR_) logger.error(ouinet::util::str(__VA_ARGS__)); } while (false)
#define LOG_ABORT(...) logger.abort(ouinet::util::str(__VA_ARGS__)) 

// Standard log levels, ascending order of specificity.
enum log_level_t { SILLY, DEBUG, VERBOSE, INFO, WARN, ERROR_, ABORT };
